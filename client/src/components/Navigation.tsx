          ...
          <AnimatePresence>
            {activeDropdownContent && (
              <motion.div
                ...
                className={`absolute left-1/2 -translate-x-1/2 top-full mt-4 rounded-2xl overflow-hidden pointer-events-auto transition-all duration-300 ease-out w-95 backdrop-blur-3xl ${scrolled
                  ? 'bg-[#060709]/50 shadow-[0_8px_30px_rgba(0,0,0,0.4)]'
                  : 'bg-[#0a0b0d]/50 shadow-[0_8px_30px_rgba(0,0,0,0.4)]'
                  }`}
                onMouseEnter={() => {
                  if (hoverTimeout.current) clearTimeout(hoverTimeout.current);
                }}
