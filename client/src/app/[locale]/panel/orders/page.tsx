      ...
      <div className="grid grid-cols-1 gap-4">
        {orders.length > 0 ? (
            ...
            >
              ...
              <div className="flex items-center gap-6 relative z-10">
                <div className="w-14 h-14 rounded-2xl bg-white/5 border border-white/10 flex items-center justify-center shrink-0 shadow-sm">
                  <ShoppingCart className="w-6 h-6 text-white/50 group-hover:text-white transition-colors" />
                </div>
                <div>
                  <h3 className="font-semibold text-white text-lg font-mono tracking-tight mb-1">
                    {order.orderNumber}
                  </h3>
              ...
              <div className="flex items-center justify-between sm:gap-10 w-full sm:w-auto relative z-10">
                <div className="flex flex-col sm:items-end">
                  ...
                  <div className={`px-3 py-1 rounded-full border text-[11px] font-bold tracking-wider uppercase ${
                    order.status === 'active' || order.status === 'paid' ? 'bg-emerald-400/10 border-emerald-400/20 text-emerald-400' :
                    order.status === 'pending_payment' ? 'bg-orange-400/10 border-orange-400/20 text-orange-400' :
                    order.status === 'cancelled' ? 'bg-red-400/10 border-red-400/20 text-red-400' :
                ...
                <div className="flex flex-col items-end min-w-[80px]">
                  <span className="text-[10px] uppercase font-bold tracking-widest text-white/30 mb-1.5">{t('total')}</span>
          ...
          <div className="px-6 py-20 rounded-3xl bg-white/[0.02] border border-white/5 flex flex-col items-center justify-center text-center">
            ...
            <h3 className="text-white font-medium text-lg tracking-tight mb-2">{t('no_orders') || 'No Orders'}</h3>
            <p className="text-white/40 text-[14px] max-w-sm tracking-wide">You don't have any orders yet.</p>
          </div>
        )}
      </div>
    </div>
