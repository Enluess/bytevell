...
export default async function WebHostingPage({ params }: { params: Promise<{ locale: string }> }) {
  const { locale } = await params;
  const t = await getTranslations({ locale, namespace: "WebHosting" });

  const products = await fetchStorefrontProducts({ groupSlug: 'web-hosting' });
            ...
            return (
              ...
              >
                <div className="mb-6">
                  ...
                  <p className="text-sm text-foreground-secondary">Great for getting started</p>
                </div>
                <div className="mb-8">
