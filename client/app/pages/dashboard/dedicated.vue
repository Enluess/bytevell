<template>
  <div>
    <div class="flex items-center justify-between mb-8">
      <div>
        <h1 class="text-3xl font-bold text-white mb-2">Dedicated Sunucular</h1>
        <p class="text-zinc-400">Fiziksel sunucularınızı tam performansla yönetin.</p>
      </div>
    ...
    <div v-else-if="services.length === 0" class="bg-zinc-900/30 border border-white/5 rounded-2xl p-12 text-center">
      <Database class="w-16 h-16 text-zinc-600 mx-auto mb-4" />
      <h3 class="text-xl font-medium text-white mb-2">Dedicated Sunucunuz Bulunmuyor</h3>
      <p class="text-zinc-400 mb-6">Projeniz için maksimum güç gerekiyorsa Dedicated paketlerine göz atın.</p>
      <NuxtLink to="/servers/dedicated" class="inline-flex items-center gap-2 bg-white text-black px-6 py-3 rounded-xl font-medium hover:bg-zinc-200 transition-colors">
        Paketleri İncele
      </NuxtLink>
    </div>
    <div v-else class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
      ...
      >
        ...
        <p class="text-sm text-zinc-400 mb-6">{{ service.ipAddress || 'IP Bekleniyor' }}</p>
...
import { Database, Loader2, Plus } from 'lucide-vue-next'
...
onMounted(async () => {
  try {
    const data = await api<{ services: any[] }>('/services?type=dedicated')
