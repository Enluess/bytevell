<template>
  <div>
    <div class="flex items-center justify-between mb-8">
      <div>
        <h1 class="text-3xl font-bold text-white mb-2">VDS Sunucular</h1>
        <p class="text-zinc-400">Tüm sanal sunucularınızı yönetin.</p>
      </div>
    ...
    <div v-else-if="services.length === 0" class="bg-zinc-900/30 border border-white/5 rounded-2xl p-12 text-center">
      <Server class="w-16 h-16 text-zinc-600 mx-auto mb-4" />
      <h3 class="text-xl font-medium text-white mb-2">VDS Sunucunuz Bulunmuyor</h3>
      <p class="text-zinc-400 mb-6">Yüksek performanslı VDS sunucularımızı keşfedin ve hemen sipariş verin.</p>
      <NuxtLink to="/servers/vps" class="inline-flex items-center gap-2 bg-white text-black px-6 py-3 rounded-xl font-medium hover:bg-zinc-200 transition-colors">
        Paketleri İncele
      </NuxtLink>
    </div>
...
import { Server, Loader2, Plus } from 'lucide-vue-next'
...
onMounted(async () => {
  try {
    const data = await api<{ services: any[] }>('/services?type=vps')
